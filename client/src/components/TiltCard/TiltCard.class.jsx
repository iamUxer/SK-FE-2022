...
export class TiltCard extends React.Component {
  ...
  // → 코드 작성
  componentDidMount() {
    const { current: domNode } = this.cardRef;
    VanillaTilt.init(domNode, this.props.options);
    domNode.addEventListener('tiltChange', this.callbackTiltEffect);
  }
  ...
  componentWillUnmount() {
    const { current: domNode } = this.cardRef;
    domNode.removeEventListener('tiltChange', this.callbackTiltEffect);
